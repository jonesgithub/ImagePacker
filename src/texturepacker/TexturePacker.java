...
public class TexturePacker {
	...
	public void pack (File outputDir, String packFileName) {
		...
		int dotIndex = packFileName.indexOf('.');
		if (dotIndex != -1 ) packFileName = packFileName.substring(0, dotIndex);
		...
		for (float scale : settings.scale) {
			...
			try {
				System.out.println("--Start writing pack file.");
				writePackFile(outputDir, pages, scaledPackFileName);
				writePlistFile(outputDir, pages, scaledPackFileName);
	...
	private void writePackFile (File outputDir, Array<Page> pages, String packFileName) throws IOException {
		if ( !packFileName.endsWith(".pack") )
			packFileName = packFileName + ".pack";
...
//				System.out.println("rect.name = " + rect.name);
	...
	private void writePlistFile (File outputDir, Array<Page> pages, String plistFileName) throws IOException {
		if(!plistFileName.endsWith(".plist")) plistFileName = plistFileName + ".plist";

		File packFile = new File(outputDir, plistFileName);
		
		System.out.println("--Start writing plist file ");

        if(pages.size >= 1)
        {
            PlistWriter writer = new PlistWriter();
            writer.outputPath = outputDir.getAbsolutePath()+ "/" + plistFileName;
            writer.fileName = plistFileName;
            
            Page page = pages.get(0);
            for(Rect rect : page.outputRects)
            {
                writer.imagerectArray.add(rect);
            }
            
            writer.info.format = 3;
            String pagesize = String.format("{{%d,%d}", page.width,page.height);
            writer.info.page = page;
          
            writer.startCreatPlist();
        }
    }
     ...
     * Author Ruizhi Du
     */
    static public class PlistWriter
    {
        PlistWriter()
        {
            imagerectArray = new ArrayList<Rect>();
            rootElement =  DocumentHelper.createDocument();
        ...
        public ArrayList<Rect> imagerectArray;
        
        public Document rootElement;
        
        public void startCreatPlist()
        {
            try
            {
                
                Element firstElement = rootElement.addElement("plist");
                Element plistElement = firstElement.addElement("dict");
                
                createFrameXMLFromArray(imagerectArray,plistElement);
                createMetaDataXml(info,plistElement);
                ...
                xmlWriter.flush();
                xmlWriter.close();
                System.out.println("xml output done");
            }
            catch (IOException e)
            {
                e.printStackTrace();
        ...
        private void createMetaDataXml(PlistInfo info, Element rootElement)
        {
            Element keyEle = rootElement.addElement("key");
            keyEle.setText("metadata");
            
            Element dictElement = rootElement.addElement("dict");
            
            Element formatKey = dictElement.addElement("key");
            formatKey.setText("format");
            Element formValue = dictElement.addElement("integer");
            formValue.setText(String.valueOf(info.format));
            
            int w = info.page.width;
            int h = info.page.height;
            int i = 1;
            while (w > 0) {
                w = w >> 1;
                i = i << 1;
            ...
            h = i;
            String size = String.format("{%d,%d}", w, h);
            Element sizeKey = dictElement.addElement("key");
            sizeKey.setText("size");
            Element sizeValue = dictElement.addElement("string");
            sizeValue.setText(size);
        }
        
        
        private  void createFrameXMLFromArray(ArrayList<Rect>  array,Element rootElement)
        {
      	  
            Element rootkey = rootElement.addElement("key");
            rootkey.setText("frames");
            Element arrayValue = rootElement.addElement("dict");
            
            for(Rect rect : array)
            {
                Element key = arrayValue.addElement("key");
                key.setText(rect.name);
                
                Element dictElement = arrayValue.addElement("dict");
                Element frameKey = dictElement.addElement("key");
                frameKey.setText("alias");
                dictElement.addElement("array");
				
                Element spriteSizeKey = dictElement.addElement("key");
                spriteSizeKey.setText("spriteSize");
                Element spriteSizeValue = dictElement.addElement("string");
                String size = String.format("{%d,%d}", rect.regionWidth ,  rect.regionHeight);
                spriteSizeValue.setText(size);
                ...
                Element spriteSourceSizeKey = dictElement.addElement("key");
                spriteSourceSizeKey.setText("spriteSourceSize");
                Element spriteSourceSizeValue = dictElement.addElement("string");
                String sourcesize = String.format("{%d,%d}", rect.originalWidth,rect.originalHeight);
                spriteSourceSizeValue.setText(sourcesize);

                Element textureRectKey = dictElement.addElement("key");
                textureRectKey.setText("textureRect");
                Element textureRectValue = dictElement.addElement("string");
                String textureRectsize = String.format("{{%d,%d},{%d,%d}}", (info.page.x + rect.x), (info.page.y + info.page.height - rect.height - rect.y), rect.regionWidth ,  rect.regionHeight);
                textureRectValue.setText(textureRectsize);

                Element rotatedKey = dictElement.addElement("key");

                rotatedKey.setText("textureRotated");
                if(rect.rotated)
                {
                    dictElement.addElement("true");
                }
                else
                {
                    dictElement.addElement("false");
                }
	...
	static public class Settings {
		...
		public int paddingX = 0, paddingY = 0; // changed by Ruizhi Du
		...
		public boolean stripWhitespaceX = true, stripWhitespaceY = true; // changed by Ruizhi Du from default value
		...
		public boolean useIndexes = false; //change from true to false by xihu
